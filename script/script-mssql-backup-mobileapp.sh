...
#
# If you wanted to automate backups, here is a cron job example (create a new file under /etc/cron.d eg. '/etc/cron.d/mssql-backups' and copy into the following line) -This will backup all your datab$
# 15 0 * * * root /usr/bin/mssqlbackup "/var/dbbackups" 30 "localhost" "sa" "<YourPasswordHere>"
#

# Set some default values
MSSQL_HOST="localhost"
BACKUP_DIR="/opt/dbbackups"
...
then
  ...
  GNU nano 2.9.3                                                                           schedule-backup-mssql.sh                                                                                     
...
for DBNAME in $DATABASES; do
        ...
        $MSSQL_EXEC -H "$MSSQL_HOST" -U "$MSSQL_USER" -P "$MSSQL_PASS" -Q "BACKUP DATABASE [${DBNAME}] TO  DISK = '${BACKUP_DIR}/${DBNAME}_${DATE}.bak' GO"
        echo "Done!"
zip ${DBNAME}_${DATE}.zip ${DBNAME}_${DATE}.bak
rm -f  ${BACKUP_DIR}/${DBNAME}_${DATE}.bak
#aws s3 cp ${DBNAME}_${DATE}.zip s3://mobileapp-backup/database --region ap-southeast-1 --delete --exclude "*.sh"
done
...
echo "syncing to AWSS3"
aws s3 sync . s3://mobileapp-backup/database --region ap-southeast-1 --delete --exclude "*.sh"
